...
available_analyses=['treeLength','RC','Sharp','tmrcas','transitions','subtrees', 'tipdate'] ## analysis names that are possible
...
for line in treefile: ## iterate through each line
    ...
    if cerberus is not None: ## tree identified
        ...
        if treecount==0: ## At tree state 0 insert header into output file
            ...
            if lower==0 and upper==np.inf: ## only add a header if not doing a chunk
                ...
                if 'subtrees' in analyses:
                    import copy
                ###########################################
                if 'tipdate' in analyses:
                    tips_sampled = ['LIBR16393-IndivA_XXXX_XXX_2015-11-19'] #List of tip names for which dates were sampled
                    for each_tip in tips_sampled:
                        outfile.write('\t%s' % each_tip)
                ###########################################
        ...
        if int(cerberus.group(1)) >= burnin and lower <= int(cerberus.group(1)) < upper: ## After burnin start processing
            ...
            #### calibration
            dateCerberus=re.compile('\_([0-9\-]+)$') ## search pattern + brackets on actual calendar date
...
#                                     outfile.write('\t%s'%(subtreeString))
            ###################################################
            if 'tipdate' in analyses:
                datediff={x:None for x in tips_sampled}
                assert calibration==True,'This analysis type requires time-calibrated trees'
                ll.renameTips(tips)
                for k in ll.Objects: ## iterate over branches
                    if isinstance(k,bt.leaf) and k.name in tips_sampled: ## only interested in tips
                        trueDate = bt.decimalDate(k.name.split('_')[-1])
                        xp=k.parent.absoluteTime
                        datediff[k.name] = (trueDate-xp)/(k.absoluteTime-xp)
                for each_tip in tips_sampled:
                    outfile.write('\t%.2f'%(datediff[each_tip]))
            ###################################################
